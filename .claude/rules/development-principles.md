...
- `src/services/base/` — 독립 도메인 서비스 (PageRenderer, ContentExtractor). 다른 서비스에 의존하지 않는다.
- `src/services/converter/` — 단일 페이지 변환 facade (wtm, WtmConfig)
- `src/services/batch/` — URL 목록 배치 변환 (WtmBatchConverter, BatchConvertConfig)
- `src/services/crawler/` — 링크 추적 크롤링 오케스트레이션 (WtmCrawler, WtmFileWriter, UrlScopeFilter)
...
- `src/services/converter/wtm.ts`, `src/services/batch/wtm-batch.ts`, `src/services/crawler/wtm-crawl.ts`가 composition root로서 의존성을 조립한다.
